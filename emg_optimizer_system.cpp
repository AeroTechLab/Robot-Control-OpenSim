...
void EMGOptimizerSystem::PreProcessSamples()
{
...
#ifdef OSIM_LEGACY
      actuatorsList[ jointIndex ]->setOverrideForce( state, torqueSample[ torqueOutputsIndex + EMG_TORQUE_EXT ] );
#else
      actuatorsList[ jointIndex ]->setOverrideActuation( state, torqueSample[ torqueOutputsIndex + EMG_TORQUE_EXT ] );
#endif
    }
  ...
  for( int muscleIndex = 0; muscleIndex < muscleSet.getSize(); muscleIndex++ )
#ifdef OSIM_LEGACY
    muscleSet[ muscleIndex ].setDisabled( state, false );
#else
    muscleSet[ muscleIndex ].setAppliesForce( state, true );
#endif
}
