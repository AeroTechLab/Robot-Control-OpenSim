...
Curve CurveInterpolation_LoadCurve( DataHandle curveData )
{
  static char filePath[ DATA_IO_MAX_FILE_PATH_LENGTH ];
  
  if( curveData == NULL ) return NULL;
  
  char* curveName = DataIO.GetDataHandler()->GetStringValue( curveData, NULL, "" );
  if( curveName != NULL )
  {
    sprintf( filePath, "curves/%s", curveName );
    if( (curveData = DataIO.GetDataHandler()->LoadFileData( filePath )) == NULL ) return NULL;
  }
  
  Curve newCurve = (Curve) malloc( sizeof(CurveData) );
  memset( newCurve, 0, sizeof(CurveData) );
  
  newCurve->scaleFactor = DataIO.GetDataHandler()->GetNumericValue( curveData, 1.0, "scale_factor" );
  newCurve->maxAbsoluteValue = DataIO.GetDataHandler()->GetNumericValue( curveData, -1.0, "max_amplitude" );

  size_t segmentsNumber = DataIO.GetDataHandler()->GetListSize( curveData, "segments" );

  for( size_t segmentIndex = 0; segmentIndex < segmentsNumber; segmentIndex++ )
  {
    double curveBounds[ 2 ];
    curveBounds[ 0 ] = DataIO.GetDataHandler()->GetNumericValue( curveData, 0.0, "segments.%lu.bounds.0", segmentIndex );
    curveBounds[ 1 ] = DataIO.GetDataHandler()->GetNumericValue( curveData, 1.0, "segments.%lu.bounds.1", segmentIndex );

    int parametersNumber = (int) DataIO.GetDataHandler()->GetListSize( curveData, "segments.%lu.parameters", segmentIndex );
    ...
    for( int parameterIndex = 0; parameterIndex < parametersNumber; parameterIndex++ )
      curveParameters[ parametersNumber - parameterIndex - 1 ] = DataIO.GetDataHandler()->GetNumericValue( curveData, 0.0, "segments.%lu.parameters.%d", segmentIndex, parameterIndex );

    char* curveType = DataIO.GetDataHandler()->GetStringValue( curveData, "", "segments.%lu.type", segmentIndex );
  ...
  if( curveName != NULL ) DataIO.GetDataHandler()->UnloadData( curveData );
